      ...
      <main class="content">
        ...
        <div class="container">
          <div class="wrap">
            ...
            <?php
            $vql = "SELECT nurse.Name_With_Initials
              FROM nurse
              INNER JOIN donation_records ON nurse.Nurse_ID =donation_records.Nurse_ID";
            ...
            $vql = "SELECT * FROM donation_records Where Hospital_ID ='$y' AND End_donation='1' GROUP BY Batch,Donation_date ORDER BY Donation_date DESC";
            $result = $conn->query($vql);
            if ($result->num_rows > 0) {
              ...
              while ($row = $result->fetch_assoc()) {
                $status = $row['AddStatus'];
                $status_class = ($status === '0') ? 'pending-row' : '';

                echo "<tr class='$status_class'><td>" . $row["Donation_date"] . "</td><td>" . $row["Batch"] . "</td><td>" . $z . "</td><td><form method='POST' action ='DonationNotify.php'>
