...
if (isset($_SESSION["ID"])) {
  ...
  $sql = "SELECT * FROM blood_testing_result Where MLT_ID='$x' ";
            ...
            <div class="midiv">
              ...
              if ($result->num_rows > 0) {
                while ($row = $result->fetch_assoc()) {
                  echo "<form method='POST' action ='View approval of blood test results2Search.php'>
    <input type=hidden name=Requestdate value=" . $row["process_date"] . " >
    <input type=hidden name=Requestbatch value=" . $row["batch_number"] . " >
    <button type=submit value=BtnSubmit id=search name=BtnSubmit  class='b1'><b>Search</b></button>
    </form>";
                }
              }
              ?>
        ...
        <div class="box">
          ...
          if (isset($_POST['BtnSubmit'])) {
            ...
            $search = $_POST["search"];
            $data = $_POST["data"];
            $sql = "SELECT * FROM blood_testing_result WHERE $search = '$data' AND process_date ='$did' AND MLT_ID='$x' AND batch_number='$batchid'";
            $result = $conn->query($sql);
