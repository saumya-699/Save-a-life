    ...
    if ($result->num_rows == 1) {
        ...
        if ($oldpwd === $row['Password']) {
            ...
            if ($stmt->affected_rows == 1) {
                ...
                $stmt = $conn->prepare("UPDATE bloodbank_doctor SET Password = ? WHERE UserName = ?");
                ...
                if ($stmt->affected_rows == 1) {
                    ...
                    echo "<script>alert('Password updated successfully.'); window.location.href = 'profileBI.php';</script>";
                    exit();
                } else {
