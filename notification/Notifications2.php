            ...
            $conn->close();
              ?>
                ...
                while ($row = $result->fetch_assoc()) {
                  ...
                  echo  "<table border=1>" . "<tr class='$status_class'>" . "<td>" . "Cross Matching testing result of request id " . $row["Request_ID"] . " on " . $row["process_date"]  ."<td>"."<form method='POST' action ='crossmatchingNotify.php'>
                  <button type=submit value=view name=view  class='fp'><i class='fa-solid fa-arrow-right'></i></button>
                  <input type=hidden name=TestID value=" . $row["test_result_id"] . " >
                  </form> "."</td>"."</tr>" . "</table>";
                }
